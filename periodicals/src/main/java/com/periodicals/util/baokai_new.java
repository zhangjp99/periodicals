...
public class baokai_new
{
  ...
  private static Logger log = Logger.getLogger(baokai_new.class.getClass());
  
  public static void main(String[] agrs)
  {
    baokai_new bk = new baokai_new();
    rwfile rwf = new rwfile();
    String fileloc="d:\\result_xw_2020bib";
...
//      log.info(bib + "\t" + title + "\t" + ISSN + "\t" + DGH + "\t" + FLH + "\t" + SQH);
//    log.info(bib);
//      int begindate = 18262;  //20200101
//      int enddate = 18414;  //20200601
...
//     int begindate = 16801;  //20160101
//     int enddate = 17166;  //20161231
  
      int begindate = 18262;  //20150101
      int enddate = 18568;  //20151231
      ...
      String fbrec = bk.GetFBrec2(bib,map.get("copyId")==null ? 0 : Integer.parseInt(map.get("copyId").toString()),title);
//      for (int i = 0; i < fbrec.length; i++) {
//    	  fbinfo += fbrec[i];
//      }
      
//      log.info(bib + "\t" + title + "\t" + ISSN + "\t" + DGH + "\t" + FLH + "\t" + SQH + "\t" + fbinfo);
      try
      {
        //rwf.AddtoFile(bib + "\t" + title + "\t" + ISSN + "\t" + DGH + "\t" + FLH + "\t" + SQH + "\t" + fbinfo, filename);
        String sql = "insert into periodicals_log values("+bib+",'"+checkString(title)+"','"+checkString(ISSN)+"','"+checkString(DGH)+"','"+checkString(FLH)+"','"+checkString(SQH)+"','','"+checkString(fbinfo)+"', getdate(),'xw','"
    ...
    for (Map<String, Object> map : list) {
    	...
    	try {
    		...
    		if(!startEnum.trim().isEmpty() && !endEnum.trim().isEmpty()) {
    			...
    			String date ="";
    			if(!DateUtils.addEYear("19700101", startDate).equals(DateUtils.addEYear("19700101", endDate))) {
    				date = " ("+DateUtils.addEYear("19700101", startDate)+" - "+DateUtils.addEYear("19700101", endDate)+")";
    			}else {
    				date = " ("+DateUtils.addEYear("19700101", startDate)+")";
    			}
    			mainClass += str+count+date+"\r";
//    			mainClass +=staStr+" - "+endStr+"; "+count+" ("+DateUtils.addEYear("19700101", startDate)+" - "+DateUtils.addEYear("19700101", endDate)+")"+"\r";
    		}else {
    			mainClass +=freeEnum +" "+freeChron+" ("+DateUtils.addEYear("19700101", endDate)+")"+"\r";
    		}
		} catch (Exception e) {
			e.printStackTrace();
			log.info(t+"--------"+copyId);
  ...
  public String GetFBrec2(int bib,int copyId,String title)
  {
    int[][] fuben = GetFB(bib);
    ...
    else
    {
      ...
      {
    	  return   getMainClass(copyId, title);
      }
    }
    return null;
  }
...
/*
  ...
  public String GetFBrec(int copy, int begindate, int enddate)
  {
    ...
    while(sqlManager.bHasResult)
    {
//      log.info(sqlManager.FieldByName("idate") + "\t" + sqlManager.FieldByName("ldate"));
